...
public interface Cache<K, V> extends Closeable {
     ...
     * <p>If error occurs during cache access, the method will not throw an exception.</p>
     * <p>if the implementation supports asynchronous operation, the cache operation of this method
     * is asynchronous.</p>
     *
     ...
     * @return true if the key is removed successfully, false if the KV association does not exists
     * in the cache, or error occurs during cache access.
     * @see #REMOVE(Object)
     */
     ...
     * <p>
     * If the implementation does not support the specified class, the {@link
     ...
     * @throws IllegalArgumentException if the caching provider doesn't support the specified
     *                                  class.
     */
     ...
     * 获取缓存的配置信息
     * <pre>
     * Get the config of this cache.</pre>
     *
     ...
     *     如果Cache实例是本地的，它是一个本地锁，在本JVM中有效；如果是redis等远程缓存，它是一个不十
     *     分严格的分布式锁。锁的超时时间由expire和timeUnit指定。多级缓存的情况会使用最后一级做tryLock操作。
     * </pre>
     ...
     *      }
     *   }
     *   上面的代码有个潜在的坑是忘记判断if(lock!=null)，所以一般可以直接用{@link #tryLockAndRun}更加简单
     * </pre>
     ...
     * @return an AutoReleaseLock(implements java.lang.AutoCloseable) instance if success. or null
     * if the attempt fails, which indicates there is an another thread/process/server has the lock,
     ...
     * after this method
     * return. The invoke of getResultCode()/isSuccess()/getMessage()/getValue() on the result will
     * block until cache operation is completed. Call future() method on the result will get a
     * CompletionStage instance for asynchronous programming.</p>
     *
     * @param key the key
     * @return the result
     */
    ...
    /**
     * <pre>
     *     当key对应的缓存不存在时，使用loader加载。cacheNullWhenLoaderReturnNull参数指定了
     *     当loader加载出来时null值的时候，是否要进行缓存（有时候即使是null值也是通过很繁重的查询才得到的，需要缓存）。
     * </pre>
     * If there is a value associated with the key, return the value; otherwise use the loader load
     * the value and return, and then update the cache.
     *
    ...
    /**
     * <pre>
     *     当key对应的缓存不存在时，使用loader加载。cacheNullWhenLoaderReturnNull
     *     参数指定了当loader加载出来时null值的时候，是否要进行缓存（有时候即使是null值
     *     也是通过很繁重的查询才得到的，需要缓存）。expire和timeUnit指定了缓存的超时时间，
     *     会覆盖缓存的默认超时时间。
     * </pre>
     * If there is a value associated with the key, return the value; otherwise use the loader load
     * the value and return, and then update the cache.
    ...
    //////////////////////////////// 大写API //////////////////////////////////////////
    /**
     * V get(K key)这样的方法虽然用起来方便，但有功能上的缺陷，当get返回null的时候，
     * 无法断定是对应的key不存在，还是访问缓存发生了异常，所以JetCache针对部分操作提
     * 供了另外一套API，提供了完整的返回值.
     *
     * 这些方法的特征是方法名为大写，与小写的普通方法对应，提供了完整的返回值，用起来也稍微繁琐一些
     * <pre>
     ...
     * <p>if the implementation supports asynchronous operation, the cache access may not completed
     * after this method
     * return. The invoke of getResultCode()/isSuccess()/getMessage() on the result will block until
     * cache operation is completed. Call future() method on the result will get a CompletionStage
     * instance for asynchronous programming.</p>
     *
     * @param key   key with which the specified value is to be associated
     * @param value value to be associated with the specified key
     * @return the result
     ...
     * specified key is not already associated with a value, or FAIL if error occurs.
     */
